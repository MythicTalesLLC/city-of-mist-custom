...
export class CityDB extends DBAccessor {
	...
	static getTagOwnerById(tagOwnerId: string) {
		...
		if (val) { return val; }
		else { throw new Error(`Couldn't find tag owner for Id ${tagOwnerId}`); }
	}
	...
	static getBuildUpImprovements(): Improvement[] {
		...
		return list.filter(item => {
			...
			if (nameFilter.length == 1) { return true; }
			else { return !item.system.free_content; }
		});
	}
	...
	static onActorUpdate(actor: CityActor, _updatedItem: unknown, _data: unknown, _diff: unknown) {
		...
		if (actor.isDanger()) { this.refreshDangerTemplates(); }
		return true;
	}
	...
	static async onTokenDelete(token: TokenDocument<Danger>) {
		...
		if (token.actor && token.actorLink) {
			if (token.actor.hasEntranceMoves() && !token.hidden) { await token.actor.undoEntranceMoves(token); }
		}
		return true;
	}
	...
	static async onTokenUpdate(token: TokenDocument<Danger>, changes?: Record<string, unknown>, _otherStuff?: unknown) {
		...
		if (changes?.hidden === false && token.actor.hasEntranceMoves()) { await token.actor.executeEntranceMoves(token); }
		if (changes?.hidden === true && token.actor.hasEntranceMoves()) { await token.actor.undoEntranceMoves(token); }
		if (game.scenes.active != token.parent) { return; }
	...
	static async onTokenCreate(token: TokenDocument<CityActor>) {
		...
		if (type == "character" || type == "crew") { await CityHelpers.ensureTokenLinked(token.parent, token); }
		if (type == "threat") {
