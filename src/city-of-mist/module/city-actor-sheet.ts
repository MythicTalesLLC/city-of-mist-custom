...
export class CityActorSheet extends CitySheet {
	...
	override activateListeners(html: JQuery<HTMLElement>) {
		...
		html.find('.toggle-veteran-theme').on("click", this._toggleVeteranTheme.bind(this));
		...
		html.find('.veteran-slot-add').on("click", this._addVeteranSlot.bind(this));
		html.find('.veteran-slot-remove').on("click", this._removeVeteranSlot.bind(this));
	...
	async _deleteImprovement(event: JQuery.Event) {
		...
		if (!impId) { throw new Error("No improvement Id"); }
	...
	async _improvementEdit(event: JQuery.Event) {
		...
		if (!imp.system.chosen) { await imp.reloadImprovementFromCompendium(); }
	...
	async _toggleVeteranTheme(event: JQuery.ClickEvent) {
		...
		await this.actor.toggleVeteranTheme(themeId);
	}

	async _addVeteranSlot(event: JQuery.ClickEvent) {
		event.preventDefault();
		if (!this.actor.isPC()) {
			return;
		}
		await this.actor.increaseMainThemeCapacity();
	}

	async _removeVeteranSlot(event: JQuery.ClickEvent) {
		event.preventDefault();
		if (!this.actor.isPC() || !this.actor.canRemoveVeteranSlot) {
			return;
		}
		await this.actor.decreaseMainThemeCapacity();
	}
	...
	async _sendImprovementToChat(event: JQuery.Event) {
		...
		if (await this.sendToChatBox(localizeS(impName).toString(), html, options)) {
			if (uses < 9999) { await imp.decrementImprovementUses(); }
		}
	}

	async _activeExtraChange(_event: JQuery.Event) {
		if (this.actor.system.type != "character") { return; }
		...
		if (this.actor.system.activeExtraId != val) {
			...
			if (extra) { await CityHelpers.modificationLog(this.actor, `Activated Extra ${extra.name}`); }
			else { await CityHelpers.modificationLog(this.actor, `deactivated extra Theme`); }
		}
	}
	...
	async _useClue(event: JQuery.Event) {
		...
		const owner = this.getOwner(actorId);
		const clue = owner.getClue(clue_id)!;
		if (await this.confirmBox("Use Clue", "Use Clue?")) { await clue.spendClue(); }
	}
