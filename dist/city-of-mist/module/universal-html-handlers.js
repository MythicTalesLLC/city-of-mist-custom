...
export class HTMLHandlers {
    ...
    static async createStatus(event) {
        ...
        const owner = CityHelpers.getOwner(ownerId, tokenId, sceneId);
    ...
    static async createStoryTag(event) {
        event.stopPropagation();
        event.preventDefault();
        const ownerId = HTMLTools.getClosestData(event, "ownerId");
        const tokenId = HTMLTools.getClosestData(event, "tokenId");
    ...
    static async deleteTag(event, autoDelete = false) {
        ...
        const removeImprovement = tag.isWeaknessTag()
            && tag.theme.weaknessTags.length >= 2
            ? (CitySettings.autoAwardImpForWeakness()
                || await HTMLTools.confirmBox(localize("CityOfMist.dialog.deleteTag.confirmExtraImprovementOnWeakness.title"), localize("CityOfMist.dialog.deleteTag.confirmExtraImprovementOnWeakness.body"), { onClose: "reject" })) : false;
