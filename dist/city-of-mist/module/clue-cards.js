...
export class ClueChatCards {
    ...
    */
    ...
    static clueEditButtonHandlers(_app, html, _data) {
    ...
    static async clue_bankClue(html) {
        ...
        if (!actor) {
            throw new Error(`Couldn't find Actor ${actorId}`);
        }
    ...
    static async clue_partialClueCheckbox(html) {
        ...
        if (this.clickLock) {
            return;
        } //this handler seems to fire multiple times for some reason
        this.clickLock = true;
        if (answer) {
