...
export class StatusMath {
    static merge(st1, st2tier) {
        return this.add(st1, st2tier);
    }
    static add(st1, st2tier) {
    ...
    static addCoMCommutative(st1, st2tier) {
        let ntier = st2tier;
        let { tier, pips = 0 } = st1;
        if (ntier > tier) {
    ...
    static addMistEngine(st1, st2tier) {
        let ntier = st2tier;
    ...
    static #convertToBinaryPips(st) {
        return (st.pips ?? 0) + (st.tier > 0 ? 1 << (st.tier - 1) : 0);
    }
    static #convertFromBinaryPips(origpips) {
        let pips = origpips;
        let tier = 0;
        while (pips) {
            pips = pips >> 1;
    ...
    static subCoM(st1, st2tier) {
        const ntier = st2tier;
        let { tier } = st1;
        tier = Math.max(tier - ntier, 0);
        return { tier, pips: 0 };
    }
    static subMistEngine(st1, st2tier) {
    ...
    static binaryToggle(status, index) {
        let toggle = 1 << index;
        let binary = this.#convertToBinaryPips(status);
        binary ^= toggle;
