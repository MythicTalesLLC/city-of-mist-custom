...
export class CityRoll {
    ...
    #prepareModifiers() {
        ...
        const allModifiers = this.#selectedList
            .filter(x => {
            const tag = x.ownerId && x.tagId ? CityHelpers.getOwner(x.ownerId, x.tokenId).getTag(x.tagId) : null;
            if (tag != null) {
                if (tag.isBurned()) {
    ...
    */
    ...
    static diceModListeners(_app, html, _data) {
        html.on('click', '.edit-roll', (ev) => void _a._editRoll(ev));
        html.on('click', '.roll-selector-checkbox', (ev) => void _a._checkOption(ev));
        html.on('click', '.roll-modifiers .name', (ev) => void _a._strikeoutModifierToggle(ev));
        html.on('click', '.strikeout-toggle', (ev) => void _a._strikeoutModifierToggle(ev));
    }
    static showEditButton(_app, html, _data) {
        if (game.user.isGM) {
            $(html).find('.edit-roll').css("display", "inline-block");
        }
        // return true;
    }
    static addCreatedTag(options, data) {
        if (data.name) {
    ...
    static async onCreateTag(chatMsg, initialData = {}) {
    ...
    static addCreatedStatus(options, data) {
        if (data.name) {
            options.createdItems.push({
                type: "status",
                name: data.name,
    ...
    static async onCreateStatus(chatMsg, initialData = {}) {
        const options = chatMsg.rolls[0].options;
        const data = await CityDialogs.getStatusData(initialData);
        this.addCreatedStatus(options, data);
        await this._updateMessage(chatMsg.id, chatMsg.rolls[0]);
    }
    static async showEffectsList(_event, chatMsg) {
        chatMsg.rolls[0].options.forceShowPanel = true;
        await this._updateMessage(chatMsg.id, chatMsg.rolls[0]);
    }
    static async onMEEffectButton(event, chatMsg) {
        ...
        const otherArray = allowables.other.map(x => Object.fromEntries([[x, `MistEngine.rollEffects.effect.extra.${x}.name`]]));
        const allowableCreations = (allowables.status.length > 0 ? [{ "status": "CityOfMist.terms.status" }] : [])
            .concat(allowables.tag.length ? [{ "tag": "CityOfMist.terms.tag" }] : [])
            .concat(otherArray);
        let result = null;
        if (allowableCreations.length > 1) {
            const choiceData = allowableCreations.flatMap(o => Object.entries(o).map(([k, v]) => ({
                id: k,
        ...
        if (!rollOptions) {
            throw new Error("Can't find roll options in Chat Message");
        }
        switch (result) {
            case "status":
    ...
    static async ME_CreateStatus(allowables, options) {
        const initialData = {
            category: {
                label: "CityOfMist.terms.category",
                initial: allowables[0],
                choices: Object.fromEntries(allowables.map(x => [x, STATUS_CATEGORIES[x]])),
                localize: true,
            }
        };
        const data = await CityDialogs.getStatusData(initialData);
    ...
    static async ME_CreateTag(allowables, options) {
        const initialData = {
            category: {
                label: "CityOfMist.terms.category",
                initial: allowables[0],
                choices: Object.fromEntries(allowables.map(x => [x, TAG_CATEGORIES[x]])),
                localize: true,
            }
        };
        const data = await CityDialogs.getTagCreationData(initialData);
    ...
    static ME_RecoverBurn(options) {
        options.extraFeats.push("recover-burn");
    }
    static async ME_CreateClue(msg, options) {
    ...
    static ME_CreateExtraFeat(options) {
        options.extraFeats.push("extra-feat");
    }
    static async deleteCreatedItem(ev, chatMsg) {
    ...
    static async deleteExtraFeat(ev, chatMsg) {
    ...
    static async outputTagOrStatusMsg(tagOrStatusHtml) {
        const messageData = {
            speaker: ChatMessage.getSpeaker(),
    ...
    static verifyCheckedOptions(checkedOptions, roll) {
        const { power } = _a.getPower(roll.options);
        const moveId = roll.options.moveId;
        const move = CityHelpers.getMoves().find(x => x.id == moveId);
        if (!move) {
    ...
    static async _updateMessage(messageId, newRoll = null) {
        ...
        try {
            const checkedOptions = this.verifyCheckedOptions(message.getFlag("city-of-mist", "checkedOptions"), roll);
            let newContent;
            if (checkedOptions) {
    ...
    static createStoryTagHandlers(msg, html) {
