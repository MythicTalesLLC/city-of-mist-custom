...
export class RollDialog extends Dialog {
    ...
    constructor(roll, moveId, actor) {
        ...
        this.#power = 0;
        this.#modifierList =
            ReviewableModifierList.fromShortHand(SelectedTagsAndStatus.getPlayerActivatedTagsAndStatus());
        this.#options = {};
        this.#pendingJuice = [];
    }
    static instance() {
    ...
    */
    async juiceSessionHandlerFn(ownerId, direction, amount) {
        {
            ...
            await this.activateHelpHurt(owner, amount, direction, this.actor.id);
            this.updateModifierPopup(html);
    ...
    spawnJuiceSession() {
        ...
        this.#juiceSession.addNotifyHandler("pending", async (dataObj) => {
    ...
    setListeners(html) {
        ...
        $(html).find("#effect-slider")
            .on("change", () => {
            this.updateModifierPopup(html);
        });
        $(html).find("#roll-modifier-amt").on("change", () => this.updateModifierPopup(html));
        $(html).find("#roll-burn-tag").on("change", () => this.updateModifierPopup(html));
    }
    async spawnGMReview() {
        const html = this.html;
        ...
        reviewSession.addNotifyHandler("tagUpdate", async ({ itemId, ownerId, changeType }) => {
    ...
    */
    ...
    async addReviewableItem(item, amount) {
        if (this.#tagReviewSession) {
    ...
    async activateHelpHurt(owner, amount, direction, targetCharacterId) {
        let arr;
        if (direction > 0) {
            // subtype = "help";
    ...
    updateModifierPopup(html = this.html) {
        ...
        this.#options.powerModifier = Number($(html).find('#effect-slider').val() ?? 0);
    ...
    updateSliderValMax(html) {
        ...
        const clue = game.actors.contents.find(x => x.isPC()
            && x.items.find(i => i.id == itemId)).items
            .find(i => i.id == itemId);
