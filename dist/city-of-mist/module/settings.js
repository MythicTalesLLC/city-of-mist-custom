...
export class CitySettings {
    static async set(settingField, newvalue) {
    ...
    static tagCreationPowerCost() {
        return Number(this.get("tagCreationCost")) ?? 2;
    }
    static statusCreationPowerCost() {
    ...
    */
    ...
    static async refreshSystem(system) {
        if (!system) {
    ...
    static getDefaultItemIcon(itemType) {
        const key = `defaultIcon_item_${itemType}`;
        try {
            return game.settings.get("city-of-mist", key) ?? "";
        }
        catch (_e) {
            return "";
        }
     ...
     *  or an empty string if none has been set. */
    static getDefaultActorIcon(actorType) {
        const key = `defaultIcon_actor_${actorType}`;
        try {
            return game.settings.get("city-of-mist", key) ?? "";
        }
        catch (_e) {
