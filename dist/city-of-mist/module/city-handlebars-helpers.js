...
export class CityHandlebarsHelpers extends HandlebarsHelpers {
    ...
    static _cityhelpers = {
        ...
        "PCList": (_actor) => {
            return game.actors.contents.filter(x => x.isPC() && x.permission > 0);
        },
        "getHelpFor": (targetactor) => {
            return game.actors.contents.filter(x => x.isPC() &&
                x.items.find(i => i.isJuice() && i.isHelp() && i.getTarget() == targetactor)).map(x => x.items
        ...
        "statusBoxesME": function (status) {
            return StatusMath.statusBoxesME(status);
        },
        "localizeS": function (txt) {
            return localizeS(txt);
        },
        "themeCard": function (theme) {
            return SystemModule.active.themeCardTemplateLocation(theme);
        },
        "keys": function (obj) {
            return Object.keys(obj);
        },
        "dynThemeType": function (theme) {
            return theme.getThemeType();
        },
        "recordContains": function (rec, val) {
            return rec[val] != undefined;
        },
        "canImprove": function (theme) {
            return theme?.canImprove() ?? false;
        },
        "hasMotivation": function (theme) {
            return theme?.canImprove() ?? false;
        },
        "MomentOfEvolutionLocalized": function () {
            return new Handlebars.SafeString(localize(SystemModule.active.gameTerms().evolution));
        },
        "EvolutionPointsLocalized": function () {
            return new Handlebars.SafeString(localize(SystemModule.active.gameTerms().buildUpPoints));
        },
        "ColectiveTermLocalized": function () {
            return new Handlebars.SafeString(localize(SystemModule.active.gameTerms().collective));
        },
        "useMilestoneTrack": function (theme) {
            return (theme.getThemePropertyTerm("milestone").length > 0);
        },
        "getThemeProgress": function (theme, property) {
            switch (property) {
                case "crack":
